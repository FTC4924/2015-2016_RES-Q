...
public class SirHammerHolonomicDriveMotorPowerCalculator {
    public static FourWheelDrivePowerLevels CalculatePowerForArcadeInputs(SirHammerDriverHolonomicInputs inputs) {
        ...
        if (inputs.slowMode) {
            levels.backLeft = levels.backLeft / 4.0f;
            levels.frontLeft = levels.frontLeft / 4.0f;
            levels.backRight = levels.backRight / 4.0f;
            levels.frontRight = levels.frontRight / 4.0f;
        }
        return levels;
    }
